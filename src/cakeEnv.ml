...
let mk_init_st () = mkApp (get_constant "cake.init_st", [| nat_type (); mk_O () |])
...
let apply_extend_dec_env new_env sem_env =
  mkApp (mk_extend_dec_env (), [| val_type (); new_env; sem_env |])
...
let mk_empty_sem_env () = mkApp (get_constant "cake.empty_sem_env", [| val_type () |])
