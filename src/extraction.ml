...
let rec translate_term env term =
  ...
  | Lambda (name,typ,body) ->
    ...
    if isSort typ || Reduction.is_arity env typ  then translate_term env' body
  ...
  | Case (ci,u,params,p,iv,c,brs) ->
    ...
    let (info,_,_,matched_term,branches_as_functions) = Inductive.expand_case env (destCase term) in
    let extract_branch i =
      ...
      let args, term = Term.decompose_lam_n (info.ci_cstr_nargs.(i)) branches_as_functions.(i) in

      let env' = List.fold_right Environ.push_rel
          (List.map (fun (n,t) -> Context.Rel.Declaration.LocalAssum (n,t)) args)
          env in
      let cake_exp = translate_term env' term in

      let arg_names = List.map (fun x -> x |> fst |> Context.binder_name |> name_to_str) args |>
                      List.rev in
...
and translate_constructor env constructor args =
  let constructor_name = fst (destConstruct constructor) in
  if Inductiveops.constructor_nallargs env constructor_name = Array.length args then
    ...
    let eta_expanded_term = Reduction.eta_expand env constructor constructor_type in
    let beta_reduced_term = Reduction.beta_appvect eta_expanded_term args in
...
and translate_fixpoint env fix_name fix_body let_body =
  match Constr.kind fix_body with
  | Lambda (name,typ,body) ->
    let env' = Environ.push_rel (LocalAssum (name,typ)) env in
    if isSort typ then translate_fixpoint env' fix_name body let_body
    else
...
let rec translate_ast_t env typ =
  ...
  | Rel i ->
    ...
    begin
      ...
      | Name id ->
        mkApp (get_ast_t_constr "Atvar", [| id |> Names.Id.to_string |> fix_type_name |> str_to_coq_str |])
    end

  | App (hd, args) ->
    if isInd hd then
      let ((mut_name,type_index),_) = destInd hd in
  ...
  | Ind ((mut_name,type_index),_) ->
    ...
    mkApp(get_ast_t_constr "Atvar", [| str_to_coq_str type_name |])
...
let translate_constructor_declaration env name (ctxt : rel_context) nb_args =
...
let translate_one_body env one_body =
  ...
  let constructors = List.map (fun (name,(args,nb)) ->
      translate_constructor_declaration (Environ.push_rel_context args env) name args nb)
      name_arg_nb
  in
  ...
  let params = List.map (fun pt ->
                         pt |> Context.Rel.Declaration.get_annot |>
                         Context.binder_name |>
                         name_to_str |>
...
let translate_declaration env const_name const_body =
  let open Declarations in
  match const_body with
  | Def const -> translate_let_declaration env const_name const
...
let translate_and_print r =
  ...
  | ConstRef const_name ->
    ...
    begin
     match const_body.const_body with
      | Def const ->
        begin
          try print_constr (translate_declaration global_env const_name const_body.const_body) with
  ...
  | IndRef (name,_) | ConstructRef ((name,_),_) ->
    ...
    print_constr (translate_type_definition global_env mut_body)
...
let translate_and_add_to_global_environment r =
  ...
  | ConstRef const_name ->
    ...
    begin
      ...
      | Def const ->
        begin
          try let cake_declaration = translate_declaration global_env const_name const_body.const_body in
            ...
            let _ = Declare.declare_definition
                               ...
                               ~cinfo:(Declare.CInfo.make ~name:(Names.Id.of_string name) ~typ:(Some (EConstr.of_constr dec_type)) ())
                               ~opaque:false
                               ~body:(EConstr.of_constr cake_declaration)
                               Evd.empty
            in
            ()
